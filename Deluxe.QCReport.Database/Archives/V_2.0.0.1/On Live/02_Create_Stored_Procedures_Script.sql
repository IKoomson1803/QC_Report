...
/****** Object:  StoredProcedure [bward].[ins_LogSystemActivity]    Script Date: 04/10/2019 12:40:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [bward].[ins_LogSystemActivity]
	-- Add the parameters for the stored procedure here
	@Category VARCHAR(20),
	@Source VARCHAR(255),
	@Details VARCHAR(MAX),
...
BEGIN
	...
	BEGIN TRY
	    INSERT INTO [bward].[SystemActivityLog](Category,Source,Details,UserId,IPAddress)
	    SELECT @Category,LTrim(RTrim(@Source)),LTrim(RTrim(@Details)),@UserId,@IPAddress
    END TRY
    BEGIN CATCH
        SET @errorMsg = 'Could not insert into bward.UserActivityLog table: ' + ERROR_MESSAGE()
        RAISERROR(@errorMsg, 16, 1)
    END CATCH
...
/****** Object:  StoredProcedure [bward].[ins_LogUserActivity]    Script Date: 07/07/2016 11:34:25 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [bward].[ins_LogUserActivity]
	-- Add the parameters for the stored procedure here
	@Category VARCHAR(20),
...
BEGIN
	...
	BEGIN TRY
	    INSERT INTO [bward].[UserActivityLog](Category,ActivityType,Source,Details,UserId,IPAddress)
	    SELECT @Category,@ActivityType,@Source,@Details,@UserId,@IPAddress
    END TRY
    BEGIN CATCH
        SET @errorMsg = 'Could not insert into bward.UserActivityLog table: ' + ERROR_MESSAGE()
        RAISERROR(@errorMsg, 16, 1)
...
/****** Object:  StoredProcedure [bward].[sel_GetUserByUsername]    Script Date: 07/07/2016 11:37:03 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [bward].[sel_GetUserByUsername]
	-- Add the parameters for the stored procedure here
   @Username CHAR(20)
AS
BEGIN
	...
	BEGIN TRY
	 SELECT * FROM  [bward].[qcUsers] u
	 INNER JOIN [bward].[qcLocation] l
	 ON u.qcUserID = l.pkey
	 WHERE u.UserName = LTRIM(RTRIM(@Username))
	 AND u.Deleted IS NULL  
    END TRY
    BEGIN CATCH
        SET @errorMsg = 'Could not get user details: ' + ERROR_MESSAGE()
        RAISERROR(@errorMsg, 16, 1)
    END CATCH
...
/****** Object:  StoredProcedure [bward].[ins_up_InsertOrUpdateUser]    Script Date: 07/07/2016 11:39:32 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [bward].[ins_up_InsertOrUpdateUser]
	-- Add the parameters for the stored procedure here
    @qcUserID INT = 0,
	@Location INT,
	@UserName CHAR(20),
    @UserNameText CHAR(20), 
    @FullName CHAR(50),
...
BEGIN
	...
	BEGIN TRY
	   ...
	   IF @qcUserID = 0 
	     BEGIN
	        INSERT INTO [bward].[qcUsers]([Location],UserName,FullName,Phone,SecurityLevel,Deleted)
	        SELECT @Location, @UserNameText, @FullName, @Phone, @SecurityLevel, @Deleted 
	     END
	   ELSE 
	     BEGIN
	     
	       
	        UPDATE [bward].[qcUsers]
    ...
    END TRY
    BEGIN CATCH
             SET @errorMsg = 'Could not insert or update Users tables: ' + ERROR_MESSAGE()
        RAISERROR(@errorMsg, 16, 1)
    END CATCH
...
/****** Object:  StoredProcedure [dbo].[sel_GetUsers]    Script Date: 07/07/2016 11:37:38 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sel_GetUsers]
	-- Add the parameters for the stored procedure here
   AS
BEGIN
	...
	BEGIN TRY
	   SELECT LTRIM(RTRIM(u.UserName)) from [bward].[qcUsers] u
	   INNER JOIN [bward].[Role] ur
	   ON u.SecurityLevel  = ur.Id
	   WHERE u.Deleted IS NULL AND u.UserName IS NOT NULL
	   ORDER BY u.UserName
