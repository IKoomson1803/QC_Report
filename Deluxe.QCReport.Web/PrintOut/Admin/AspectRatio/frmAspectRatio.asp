...
'********************
dim rsFrm, rsCombo
dim sAspectId, sAspectRatio
dim sql
dim sBGColor, intItem_num
'********************

sAspectId = Request.QueryString("sAspectID")
sAspectRatio = Request.QueryString("sAspectRatio")

Response.Write "<font size=2><b>" & sAspectID & "</b></font>"
...
'*******************************************************************************************************
		sql="SELECT AspectID, AspectRatio FROM optAspectRatio " & _
		"ORDER BY AspectID"
'*******************************************************************************************************
 ...
 %>
 <b>Aspect Ratio Drop-Down Contents</b>
<CENTER>
<BR>
<table id="tblForm" width=100%>
	<tr>
		<td>
			...
			<form method="post" name="frmAspectRatio" action="procAspectRatio.asp">
				<tr>
					<td>
						Aspect Ratio:</td>
					<td>				
						<input type="text" id="txtAspectRatio" name="txtAspectRatio" size="10">
					</td>
				</tr>
				<tr>
					<td>&nbsp</td>
				...
				<tr>
					<td align="Center" colspan="6">
						<input type=submit name=subUpdate value="Add Aspect Ratio"></td>
				</tr>
				</form>
			</table>
		</td>
		<td>
			<table align=center id="tblTapeFormat" border=0 width=200 cellpadding=0 cellspacing=0>
				...
				<%		do while not rsFrm.EOF %>
							<tr class=<%=sBGColor%> onmouseover="this.className='trWhite'" onmouseout="this.className='<%=sBGColor%>'" onclick="javascript:OpenEditFrm('UpdateAspect.asp?nAspectID=<%=rsFrm.Fields("AspectID")%>');">
								<td align="center"><%=rsFrm.Fields("AspectRatio")%></td>
							</tr>
				<%			rsFrm.MoveNext
