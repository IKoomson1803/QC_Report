...
select case Request.Form("subUpdate")
	case " Add "
	sqlLog = "INSERT INTO qcTime (QCNum, subQCNum, Time_code, QC_Code, Note, QC_Grade, Action, item_Duratn, in_Master) VALUES (" & clng(sQCNum) & ", " & cint(sRev) & ", '" & trim(sTC) & "', '" & sType & "', '" & replace(sDesc, "'", "''") & "', " & cint(sGrade) & ", '" & sSect & "', '" & replace(sDuratn, "'", "''") & "', " & cint(sMaster) & ")"
	case "Save"
